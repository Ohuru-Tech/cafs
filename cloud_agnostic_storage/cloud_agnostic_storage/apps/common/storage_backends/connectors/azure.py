...
class AzureStorageFile(File):
    ...
    def write(self, content):
        if (
            "w" not in self._mode
            and "+" not in self._mode
            and "a" not in self._mode
        ):
...
def _get_valid_path(s):
    ...
    if not len(s):
        raise ValueError(
            "File name must contain one or more printable characters"
        )
    if s.count("/") > 256:
...
@deconstructible
class AzureStorage(CustomBaseStorage):
    ...
    def get_name_max_len(self):
        max_len = (
            _AZURE_NAME_MAX_LEN - len(self._get_valid_path("foo")) - len("foo")
        )
