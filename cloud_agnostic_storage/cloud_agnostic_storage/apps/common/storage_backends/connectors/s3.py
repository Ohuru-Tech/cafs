...
class S3Boto3StorageFile(CompressedFileMixin, File):
    ...
    def __init__(self, name, mode, storage, buffer_size=None):
        ...
        self._mode = mode
        self._force_mode = (
            (lambda b: b) if "b" in mode else (lambda b: b.decode())
        )
        ...
        self.buffer_size = buffer_size or setting(
        ...
        if self._file is None:
            ...
            if "r" in self._mode:
                ...
                self.obj.download_fileobj(
                    self._file, Config=self._storage._transfer_config
                )
                self._file.seek(0)
            if self._storage.gzip and self.obj.content_encoding == "gzip":
                self._file = self._decompress_file(
                    mode=self._mode, file=self._file
                )
        return self._file
...
@deconstructible
class S3Boto3Storage(CompressStorageMixin, CustomBaseStorage):
    """
    ...
    def _get_security_token(self):
        ...
        """
        security_token = self.security_token or lookup_env(
            self.security_token_names
        )
    ...
    def _save(self, name, content):
        ...
        obj.upload_fileobj(
            content, ExtraArgs=params, Config=self._transfer_config
        )
        return cleaned_name
    ...
    def _strip_signing_parameters(self, url):
        ...
        filtered_qs = (
            (key, val) for key, val in qs if key.lower() not in blacklist
        )
