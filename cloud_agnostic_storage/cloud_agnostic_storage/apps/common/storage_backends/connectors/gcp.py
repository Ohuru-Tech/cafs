        ...
        if self._file is not None:
            if self._is_dirty:
                ...
                self.blob.upload_from_file(
                    ...
                    predefined_acl=blob_params.get(
                        "acl", self._storage.default_acl
                    ),
                )
            self._file.close()
            self._file = None
...
@deconstructible
class GoogleCloudStorage(CompressStorageMixin, CustomBaseStorage):
    ...
    def _save(self, name, content):
        ...
        upload_params["predefined_acl"] = blob_params.pop(
    ...
    def get_object_parameters(self, name):
        ...
        if "cache_control" not in object_parameters and self.cache_control:
            warnings.warn(
                "The GS_CACHE_CONTROL setting is deprecated. Use"
                " GS_OBJECT_PARAMETERS to set any writable blob property or"
                " override GoogleCloudStorage.get_object_parameters to vary"
                " the parameters per object.",
                DeprecationWarning,
            )
